...
Zed reads `AGENTS.md` as project instructions and picks up the committed project config under `.zed/`:

- `.zed/settings.json`: 2-space indentation for the languages used here (Zed's default is 4, unlike VS Code's 2).
- `.zed/tasks.json`: reusable tasks for dev, worker, typecheck, tests, and e2e (run from the task modal or `task: spawn` from the command palette).

Zed respects `.gitignore`, so build output (`e2e-build/`, `.output/`, `coverage/`) and worktrees are already kept out of the project browser and search. `src/routes/routeTree.gen.ts` is generated by the router plugin but checked in, and Zed has no per-file readonly setting, so the do-not-edit rule in `AGENTS.md` is the only guard there.
