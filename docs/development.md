...
```bash
bun run setup              # Installs root and agent (agent is not a workspace member)
...
```bash
bun run setup           # Installs root and agent
...
```bash
# Root only
bun test                    # Run root tests (enforces coverage thresholds)
