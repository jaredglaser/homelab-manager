...
| Agent updates | Manual (user pulls new image) | **Agent-updater sidecar** watches for new image tags, recreates agent |
| Agent per host | One per Docker host | One stack per host (agent + socket proxy + agent-updater) |

---

## Agent as Universal Sidecar

The agent evolves from a Docker-only sidecar into a universal homelab sidecar. Each host runs an **agent stack** — a docker-compose file containing the agent, a socket proxy (for Docker hosts), and an agent-updater sidecar. The UI generates the compose file with pre-filled configuration, and the user deploys it on the target host.
...
| `agent-updater` | Watches for new agent image tags and recreates the agent container | Always |
...
The agent-updater sidecar is a lightweight container that periodically checks GHCR for new agent image tags. When a new version is found, it pulls the image and recreates the agent container. The agent-updater needs the Docker socket mounted to manage sibling containers. See [Reference Agent Stack](#reference-agent-stack) for the compose file.
...
services:
  ...
  agent-updater:
    image: ghcr.io/your-org/homelab-manager-agent-updater:latest
...
- The agent-updater mounts the Docker socket directly because it needs to pull images and recreate sibling containers. This is the only container with raw socket access.
...
### Agent-Updater Sidecar

The agent-updater is a minimal container (~5MB) that lives in the monorepo alongside the agent (`agent-updater/` package):
...
The stack includes the agent, socket proxy, and agent-updater sidecar — no additional setup needed for Docker hosts.

**Quick start: One-liner (no socket proxy, no agent-updater)**
...
> **Note:** The one-liner skips the socket proxy and agent-updater. For production use, deploy the full agent stack.

### Agent Updates

The agent stack includes an **agent-updater sidecar** that automatically watches GHCR for new agent image tags and recreates the agent container (see [Agent-Updater Sidecar](#agent-updater-sidecar)). No manual intervention needed for routine updates.

For hosts using the one-liner (no agent-updater), users update manually:
...
The UI shows the current agent and agent-updater versions (from `/health`) and displays warnings when either is outdated. The web server does **not** refuse connections from old agents — availability matters more than enforcing upgrades in a homelab.
...
| Compose services | Central: `socket-proxy` + `agent` | Per-host stack: `agent` + `socket-proxy` + `agent-updater` |
...
1. Create reference agent stack compose file (agent + socket proxy + agent-updater)
2. Agent connects to socket proxy via `DOCKER_HOST=tcp://socket-proxy:2375` (same as today, but stack-local)
3. Build agent-updater sidecar — lightweight container that watches GHCR for new agent image tags and recreates the agent
...
| Agent-updater sidecar | New container image — minimal binary that watches GHCR for agent image updates. |
...
| `src/lib/services/agent-update-service.ts` | Remove Dockerode-based update. Updates handled by agent-updater sidecar. May keep version-check logic for UI warnings. |
...
|---|---------|------|------------|
| N1 | **Socket proxy in agent stack** | Socket proxy restricts Docker API surface, but the agent-updater sidecar still needs raw socket access to manage sibling containers. | Socket proxy is on an `internal: true` network — not reachable from outside the stack. Agent-updater is minimal code (~5MB image) with a narrow scope. Only the agent-updater mounts the Docker socket; the agent never does. |
...
| N3 | **Agent update lifecycle** | Users may run outdated agent versions with known vulnerabilities. | Agent-updater sidecar automatically pulls new agent images and recreates the container. UI shows version warnings for outdated agents and agent-updaters. Web server does **not** refuse connections from old agents — this is a homelab app, availability matters more than enforcing upgrades. Agent-updater verifies health after update and rolls back on failure. |
...
| N5 | **Single agent = single point of failure per host** | If the agent stack goes down, all monitoring and management for that host stops. | All stack services use `restart: unless-stopped`. Agent-updater can restart failed agent containers. Health checks detect failures and show status in UI. Acceptable for homelab use. |
...
| N7 | **Agent-updater has raw Docker socket access** | The agent-updater container mounts `/var/run/docker.sock` to manage sibling containers. A compromised agent-updater has full Docker daemon access. | Agent-updater is a minimal, single-purpose binary with no network exposure (no ports, no API). It only interacts with the Docker socket to pull images and recreate the agent container. Small codebase makes it auditable. |
