...
1. **Styling**: TailwindCSS ONLY. Never use MUI `sx` props or create `.css` files (exceptions: `App.css`, `theme.ts`). Inline `style` only when Tailwind cannot express the value (virtualizer positioning, dynamic indent, computed transforms). Never use hardcoded hex colors - use theme CSS variables. MUI overrides work by cascade layer order: `StyledEngineProvider enableCssLayer` (in `ThemeProvider.tsx`) injects emotion styles into `@layer mui`, and `App.css` declares `@layer theme, base, mui, components, utilities;` before the tailwind import, so plain utilities beat MUI styles. Never add Tailwind's `!` postfix for MUI overrides; reserve it for inline styles set by libraries (e.g. echarts-for-react sizing) or to force one utility over another on the same element (e.g. stale row tint vs hover tint). Prefer MUI's built-in component behavior (hover effects, transitions) over custom overrides unless there's a specific design requirement.
