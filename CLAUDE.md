...
- Root route (`__root.tsx`) has both `shellComponent` (plain HTML for SSR) and `component` (client-side `<AppShell>` + `<Outlet />`).
- `AppShell` renders once in the root layout and persists across navigations — do NOT wrap individual routes with `<AppShell>`.
- All routes: `ssr: false` (SPA mode), so the `component` prop is purely client-side (MUI/React safe).
...
11. **Root route layout**: The root route (`__root.tsx`) has both `shellComponent` (SSR, plain HTML only) and `component` (client-side `<AppShell>` + `<Outlet />`). Since all routes use `ssr: false`, the `component` prop is purely client-side. Do NOT wrap individual routes with `<AppShell>` — it already lives in the root layout.
...
- Wrapping individual routes with `<AppShell>` (it lives in the root layout)
