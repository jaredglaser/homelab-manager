...
Each agent reports what it runs: `resolveAgentImage()` (`agent/src/lib/agent-image.ts`) prefers the `AGENT_IMAGE` env var the generated compose passes through, else inspects its own container's `Config.Image`, and `/info` returns `agentImage`/`agentImageTag`. The health check stores both on `managed_hosts` via `updateAgentInfo`, so `bucketHostsByAgentChannel` can flag hosts off the dashboard's channel. Both fields stay null for an agent that predates image reporting; only an agent that answered `/info` is allowed to clear them, so a 404 never wipes a known tag. Moving an enrolled host still means editing the `.env` on that machine, which nothing here can do remotely, so the notice tells the operator which hosts and which two lines.
