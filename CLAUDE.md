...
Opt-in POC, off by default. Any OpenAI-compatible endpoint (local compute included) via the Vercel AI SDK's `@ai-sdk/openai-compatible` provider; every model call goes through the `TextGenerator` seam in `src/lib/ai/text-generator.ts` so tests never touch the network. One analyst per container per UTC day, keyed on `(host, container_key)` where `container_key` is the compose service key (never the Docker id, which changes on redeploy). First sight of a container triggers a profiling pass over its log history (`/logs/:id/history` on the agent) that writes a bespoke Markdown skill; the per-day analyst runs with that skill as its system prompt, files observations through tools, and keeps a rolling summary that `ContextWindow` compacts when the token estimate crosses budget. The summary lives in `ai_analysis_sessions`, so a restart resumes the day. An orchestrator loop claims untriaged observations and folds cross-container patterns into `ai_alerts`. Details: [AI Fleet Log Analysis](docs/architecture.md#ai-fleet-log-analysis-srclibai-srcworkerai).

### Database Tables

Hypertables: `docker_stats`, `zfs_stats`, `proxmox_stats`, `docker_container_events` (append-only state-change log; current snapshot via `DISTINCT ON (host, container_id) ORDER BY at DESC`, broadcast via `NOTIFY docker_container_change`). Plus `entity_metadata` (icons/labels), `settings` (KV with NOTIFY trigger), `managed_hosts` (Docker hosts with agent connection details), `deploy_history` (deploy records with status tracking), `deploy_queue` (newest git-push request blocked by an active deploy, one row per stack+host), `stack_secrets` (per-stack environment-variable secrets, JWE-encrypted at rest), `agent_keypairs` (per-host Ed25519 keypair: private JWK encrypted, public JWK as JSONB), and the AI log-analysis set: `ai_providers` (one row, API key JWE-encrypted), `ai_container_profiles` (per-container skill), `ai_analysis_sessions` (one per container per UTC day), `ai_observations` and `ai_alerts` (all four NOTIFY `ai_analysis_change`). Schema details in `migrations/`. Every migration file runs inside a single transaction; work that PostgreSQL refuses to run in a transaction block (`CALL refresh_continuous_aggregate`) belongs in a startup task, not a migration.
