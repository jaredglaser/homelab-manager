...
The `web` image carries its own channel: CI passes `VITE_AGENT_IMAGE_TAG` as a build arg to the `production` stage, `getAgentImageTag()` (`src/lib/hosts/host-utils.ts`) reads it through `import.meta.env` (Vite inlines it at build; a missing or non-Docker-grammar value falls back to `latest`), and the Add Host wizard pins both agent images to it. That covers new enrollments only. An already-enrolled host pins its tag in its own `.env` on the remote machine, which nothing here can rewrite, so `ManagedHostsCard` shows an operator-applied notice instead; per-host detection is not possible while the agent reports only its package version.
