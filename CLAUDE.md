...
1. **Styling**: TailwindCSS ONLY. Components come from `src/components/ui/` (shadcn-style, built on `@base-ui/react`, vendored source, hand-edited for parity with the old MUI look). Never use MUI `sx` props or create `.css` files (exceptions: `App.css`, `theme.ts`). Inline `style` only when Tailwind cannot express the value (virtualizer positioning, dynamic indent, computed transforms). Never use hardcoded hex colors - use the design tokens in `App.css` (`bg-card`, `text-muted-foreground`, `border-border`, `bg-level1`, `bg-chart-bg`, etc.). MUI remains ONLY in files PR #206 rewrites (most of `src/components/docker/`, `StackContainersPanel`, `BottomDrawer`) plus `ModeToggle`/`ThemeProvider`/`theme.ts`; do not add new MUI usage. The final MUI/emotion removal PR depends on #206 landing first. During coexistence the tokens alias the runtime `--mui-palette-*` vars (see `App.css`), so both systems stay in sync, including the dynamic light palette from `useLightPaletteEffect`.
...
- **UI:** shadcn-style components on Base UI (`@base-ui/react`, vendored in `src/components/ui/`) + TailwindCSS v4 (styling, via `@tailwindcss/vite` plugin - no config file) + sonner (toasts). MUI v9 remains only in the PR #206 file set and the theme bootstrap until the final removal PR.
...
- **Styling**: TailwindCSS v4 configured in `App.css` with `@import "tailwindcss"`. shadcn design tokens (`--background`, `--card`, `--popover`, `--muted`, `--accent`, `--level1-3`, `--chart-bg`, `--success`, `--warning`, `--info`, `--tooltip`, etc.) defined in `App.css`; during MUI coexistence they alias the runtime `--mui-palette-*` vars generated by `src/theme.ts` (`cssVariables` mode). Dark mode keys off the `data-color-scheme` attribute (`@custom-variant dark`). Chart CSS vars (`--chart-cpu`, `--chart-memory`, etc.) in `App.css`.
