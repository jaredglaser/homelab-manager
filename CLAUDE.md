...
- **Settings**: Jotai atoms synced via SSE (`/api/settings`). Domain-scoped hooks (`useDockerSettings`, `useZfsSettings`, `useProxmoxSettings`, `useGeneralSettings`) provide optimistic setters and subscribe via `selectAtom` so each consumer only re-renders when its own slice changes. `useSettings()` remains as a composite wrapper for the settings page. Keys in `src/lib/constants/settings-keys.ts`. `SettingsRepository.set` issues `pg_notify('settings_change', key)` and the broadcast service fans it out to all clients.
- **View state**: the keys in `VIEW_STATE_KEYS` (row expansion, per-container shell) share the `settings` table but never touch the SSE channel: a row expand would otherwise write, notify, and re-parse every setting on every connected client for each click. They load once per route (`viewStateQueryOptions` via `ensureQueryData` in the `/docker`, `/zfs`, `/proxmox`, `/stacks` loaders) and live in the query cache under `['view-state']` with `staleTime: Infinity`. `useViewState` writes with `setQueryData` and persists in the background through `setViewState`, which any authenticated role may call (`updateSetting` is admin/operator only, so routing expansion through it made every click fail for a `viewer`). A failed persist is logged, not rolled back: the cost is one collapsed row on the next reload. Domain wrappers: `useDockerViewState`, `useZfsViewState`, `useProxmoxViewState`. Docker and Proxmox store the *collapsed* entries (absence = expanded); ZFS stores the expanded ones.
