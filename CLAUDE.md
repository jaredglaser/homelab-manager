...
### Ansible Execution Layer (`ansible-sidecar/`, `src/lib/ansible/`)

Proof of concept behind `ANSIBLE_RUNNER_ENABLED` (default off); the agent deploy path is unchanged and still owns stack deploys. A Python sidecar runs `ansible-runner` as a library and exposes `POST /runs`, `GET /runs/{id}/events` (SSE), `POST /runs/{id}/cancel`, `GET /health`. The app dispatches a run, drains the sidecar's event stream into `ansible_runs`/`ansible_run_events`, and rebroadcasts over `/api/ansible-runs`. `useAnsibleRun` preloads the newest run from `ansible_run_events` via `getLatestAnsibleRun` before subscribing, then merges SSE frames deduped on the line key, so a reload, a second tab, or a restart mid-run shows the run so far instead of only what that tab witnessed live. The preload is unbounded on purpose: `normalizeRunnerEvent` applies a strict allowlist to runner job events, so nothing outside the five known shapes is persisted or streamed, and because the per-item loop events are not in it a run's event count is bounded by tasks times hosts rather than by playbook data. Extravars use `suppress_env_files=True` (nothing decrypted reaches disk) and secret-consuming tasks are `no_log`. `ANSIBLE_HOST_KEY_CHECKING` must be set explicitly: `ansible-runner` defaults it to `False` when absent. Inventory comes from `/api/ansible-inventory`, authenticated with the sidecar token rather than a session cookie. A run row is only ever written by the process that dispatched it, so stranded `pending`/`running` rows are recovered twice over, mirroring the deploy pipeline: `performAnsibleRunRecovery` at startup and `AnsibleRunWatchdog` on an interval, both publishing `run_finished` so the UI unsticks. Keep `ANSIBLE_WATCHDOG_TIMEOUT_MINUTES` above the sidecar's `ANSIBLE_IDLE_TIMEOUT_SECONDS`. Details: [Ansible Execution Layer](docs/architecture.md#ansible-execution-layer-ansible-sidecar).
