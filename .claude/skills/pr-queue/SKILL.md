...
`chmod 700` that directory: the hook prints what it finds into model context, so a world-writable
state dir on a shared host forges the pointer. The hook re-checks the issue-URL shape as well.
...
first, prune them last. `CI` must separate terminal (`pass`, `fail`) from non-terminal (`pending`,
unknown), which the no-op rule keys off. Under `Blocked on`, name the unresolved findings and their
dispositions: "blocked on review" forces a round trip, a finding at a file:line with its fix cost
does not.
...
`Head SHA`, `Updated` and `Base SHA`. Never open with a per-PR poll. But an unchanged head and
`updated_at` only mean no PR-level event fired: check runs advance on their own, and PR 378 went
from in-flight to 8/8 green with both frozen. A PR that moved gets `pull_request_read` for `get` and
`get_check_runs`; a quiet PR whose `CI` is non-terminal gets `get_check_runs` anyway; the rest get
nothing, tracker timestamp included.
...
`_Generated by [Claude Code](https://claude.ai/code)_`. No tag: it marks artifacts this session
owns, and a reply is not one.
