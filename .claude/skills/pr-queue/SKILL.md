...
CLAUDE.md rule 15 governs everything you write on GitHub, tracker issue included: no em dashes, no
en dashes, no `--` standing in for one, and none of the vocabulary tells. Use a comma, colon or
parenthesis. This is the rule you will break first, because you write far more prose in this role
...
needs no coordination and never has to be unique, since the tag already guarantees that and the tag is
...
One GitHub issue per session, titled `PR queue: <name> (pq-xxxxxxxx)`, whose body you **edit in place** so
...
CodeRabbit withdraws findings when the reply is concrete. Fix silently, since the diff is the record, and
...
concern is a decision, so proceed with the full request.
...
- **CI is the authority, not local runs.** Local `bun test` failures are usually drifted
  ...
  `get_status`, which shows only CodeRabbit and has reported green over two failing jobs.
