...
async function generateIconManifest() {
  ...
  // Assemble the marker at runtime so this generator source does not contain
  // the contiguous opt-out token; only the generated manifest needs the marker.
  const disableMarker = ['claudism-check', 'disable-file'].join(':');
  const manifest = `// Auto-generated by scripts/download-icons.ts
// Do not edit manually - run \`bun icons:download\` to regenerate
// ${disableMarker}: icon slugs include brand names that clash with rule 15 vocabulary tells.
